...
/** A resource that represents the data of a single raw artifact as digital content accessible in its native format. A Binary
  ...
  *     proxy equivalent for this purpose. This referenced resource may be related to the Binary (e.g. Media, DocumentReference),
  *     or may be some non-related Resource purely as a security proxy. E.g. to identify that the binary resource relates to a
  *     patient, and access should only be granted to applications that have access to the patient.
  */
@POJOBoilerplate
class Binary(
