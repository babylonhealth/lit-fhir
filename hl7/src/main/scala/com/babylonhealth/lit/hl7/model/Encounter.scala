...
object Encounter extends CompanionFor[Encounter] {
  implicit def summonObjectAndCompanionEncounter1049926270(
      o: Encounter): ObjectAndCompanion[Encounter, Encounter.type] = ObjectAndCompanion(o, this)
  ...
  object StatusHistory extends CompanionFor[StatusHistory] {
    ...
    def unapply(
        o: StatusHistory): Option[(Option[String], ENCOUNTER_STATUS, Period, LitSeq[Extension], LitSeq[Extension])] =
  ...
  object Diagnosis extends CompanionFor[Diagnosis] {
    implicit def summonObjectAndCompanionDiagnosis_610612379(
  ...
  object Location extends CompanionFor[Location] {
    ...
    val status: FHIRComponentFieldMeta[Option[ENCOUNTER_LOCATION_STATUS]] =
      FHIRComponentFieldMeta(
        "status",
        lTagOf[Option[ENCOUNTER_LOCATION_STATUS]],
        false,
        lTagOf[ENCOUNTER_LOCATION_STATUS])
    val period: FHIRComponentFieldMeta[Option[Period]] =
      FHIRComponentFieldMeta("period", lTagOf[Option[Period]], false, lTagOf[Period])
...
/** An interaction between a patient and healthcare provider(s) for the purpose of providing healthcare service(s) or
  ...
  *
  * @constructor
  *   Introduces the fields `type`, `class`, status, period, length, partOf, subject, basedOn, account, priority,
  *   identifier, reasonCode, serviceType, appointment, episodeOfCare, reasonReference, serviceProvider, location,
  *   diagnosis, participant, classHistory, statusHistory, hospitalization.
  * @param id
  ...
  * @param `class`
  *   - Concepts representing classification of patient encounter such as ambulatory (outpatient), inpatient, emergency,
  ...
  * @param episodeOfCare
  *   - Where a specific encounter should be classified as a part of a specific episode(s) of care this field should be
  *   used. This association can facilitate grouping of related encounters together for a specific purpose, such as
  *   government reporting, issue tracking, association via a common problem. The association is recorded on the
  *   encounter as these are typically created after the episode of care and grouped on entry rather than editing the
  *   episode of care to append another encounter to it (the episode of care could span years).
  * @param reasonReference
  *   - Reason the encounter takes place, expressed as a code. For admissions, this can be used for a coded admission
  *   diagnosis.
  * @param serviceProvider
  *   - The organization that is primarily responsible for this Encounter's services. This MAY be the same as the
  *   organization on the Patient record, however it could be different, such as if the actor performing the services
  *   was from an external organization (which may be billed seperately) for an external consultation. Refer to the
