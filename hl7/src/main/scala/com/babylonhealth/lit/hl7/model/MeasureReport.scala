...
object MeasureReport extends CompanionFor[MeasureReport] {
  ...
  object Group extends CompanionFor[Group] {
    ...
    object Population extends CompanionFor[Population] {
      ...
      val fieldsMeta: Seq[FHIRComponentFieldMeta[_]] = Seq(id, code, count, extension, subjectResults, modifierExtension)
...
/** The MeasureReport resource contains the results of the calculation of a measure; and optionally a reference to the resources
  ...
  * @param `type`
  *   - The type of measure report. This may be an individual report, which provides the score for the measure for an individual
  *     member of the population; a subject-listing, which returns the list of members that meet the various criteria in the
  *     measure; a summary report, which returns a population count for each of the criteria in the measure; or a data-collection,
  *     which enables the MeasureReport to be used to exchange the data-of-interest for a quality measure.
  * @param date
  ...
  *     of the extension.
  * @param identifier
  *   - A formal identifier that is used to identify this MeasureReport when it is represented in other formats or referenced in a
