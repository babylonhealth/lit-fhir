...
object DocumentManifest extends CompanionFor[DocumentManifest] {
  ...
  object Related extends CompanionFor[Related] {
    ...
    def unapply(
        o: Related): Option[(Option[String], Option[Reference], LitSeq[Extension], Option[Identifier], LitSeq[Extension])] =
      Some((o.id, o.ref, o.extension, o.identifier, o.modifierExtension))
...
/** A collection of documents compiled for a purpose together with metadata that applies to the collection.
  ...
  *
  * @constructor
  *   Introduces the fields `type`, status, author, source, subject, created, content, recipient, identifier,
  *   description, masterIdentifier, related.
  * @param id
  ...
  * @param author
  *   - Identifies who is the author of the manifest. Manifest author is not necessarly the author of the references
  *   included.
  * @param source
  *   - Identifies the source system, application, or software that produced the document manifest.
  * @param subject
  *   - Who or what the set of documents is about. The documents can be about a person, (patient or healthcare
  *   practitioner), a device (i.e. machine) or even a group of subjects (such as a document about a herd of farm
  *   animals, or a set of patients that share a common exposure). If the documents cross more than one subject, then
  *   more than one subject is allowed here (unusual use case).
  * @param created
  *   - When the document manifest was created for submission to the server (not necessarily the same thing as the
  *   actual resource last modified time, since it may be modified, replicated, etc.).
  * @param content
  ...
  *   - A single identifier that uniquely identifies this manifest. Principally used to refer to the manifest in
  *   non-FHIR contexts.
