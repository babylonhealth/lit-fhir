...
object ChargeItemDefinition extends CompanionFor[ChargeItemDefinition] {
  ...
  def extractUrl(t: ChargeItemDefinition): UriStr                                               = t.url.get
...
/** The ChargeItemDefinition resource provides the properties that apply to the (billing) codes necessary to calculate costs and
  ...
  *
  * @constructor
  *   Introduces the fields code, partOf, replaces, instance, derivedFromUri, applicability, propertyGroup. Requires the following
  *   fields which were optional in the parent: url.
  * @param id
  ...
  *   at which at which an authoritative instance of this charge item definition is (or will be) published. This URL can be the
  *   target of a canonical reference. It SHALL remain the same when the charge item definition is stored on different servers.
  * @param meta
  ...
  *   the charge item definition changes.
  * @param code
  ...
  *   specification, model, design or instance. This is an arbitrary value managed by the charge item definition author and is not
  *   expected to be globally unique. For example, it might be a timestamp (e.g. yyyymmdd) if a managed version is not available.
  *   There is also no expectation that versions can be placed in a lexicographical sequence. To provide a version consistent with
