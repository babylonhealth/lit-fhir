...
object HealthcareService extends CompanionFor[HealthcareService] {
  implicit def summonObjectAndCompanionHealthcareService_310532684(
      o: HealthcareService): ObjectAndCompanion[HealthcareService, HealthcareService.type] = ObjectAndCompanion(o, this)
  ...
  object AvailableTime extends CompanionFor[AvailableTime] {
    implicit def summonObjectAndCompanionAvailableTime2089229632(
  ...
  object NotAvailable extends CompanionFor[NotAvailable] {
    implicit def summonObjectAndCompanionNotAvailable_1165707934(
  ...
  object Eligibility extends CompanionFor[Eligibility] {
    implicit def summonObjectAndCompanionEligibility770862261(
        o: Eligibility): ObjectAndCompanion[Eligibility, Eligibility.type] = ObjectAndCompanion(o, this)
...
/** The details of a healthcare service available at a location.
  ...
  *
  * @constructor
  *   Introduces the fields `type`, name, photo, active, comment, telecom, program, category, location, endpoint,
  *   specialty, identifier, providedBy, extraDetails, coverageArea, communication, characteristic, referralMethod,
  *   appointmentRequired, serviceProvisionCode, availabilityExceptions, eligibility, notAvailable, availableTime.
  * @param id
  ...
  * @param active
  *   - This flag is used to mark the record to not be used. This is not used when a center is closed for maintenance,
  *   or for holidays, the notAvailable period is to be used for this.
  * @param comment
  *   - Any additional description of the service and/or any specific issues not covered by the other attributes, which
  ...
  * @param communication
  *   - Some services are specifically made available in multiple languages, this property permits a directory to
  *   declare the languages this is offered in. Typically this is only provided where a service operates in communities
  *   with mixed languages used.
  * @param characteristic
  *   - Collection of characteristics (attributes).
  * @param referralMethod
  *   - Ways that the service accepts referrals, if this is not provided then it is implied that no referral is
  *   required.
  ...
  *   to be provided by the Organization. Indicates if an appointment is required for access to this service.
  * @param serviceProvisionCode
  *   - The code(s) that detail the conditions under which the healthcare service is available/offered.
  * @param availabilityExceptions
  *   - A description of site availability exceptions, e.g. public holiday availability. Succinctly describing all
  *   possible exceptions to normal site availability as details in the available Times and not available Times.
  * @param eligibility
