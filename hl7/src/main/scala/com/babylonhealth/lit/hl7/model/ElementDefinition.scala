...
object ElementDefinition extends CompanionFor[ElementDefinition] {
  ...
  object Binding extends CompanionFor[Binding] {
    ...
    def unapply(
        o: Binding): Option[(Option[String], BINDING_STRENGTH, Option[Canonical], LitSeq[Extension], Option[String])] =
  ...
  object Type extends CompanionFor[Type] {
    ...
    val versioning: FHIRComponentFieldMeta[Option[REFERENCE_VERSION_RULES]] =
      FHIRComponentFieldMeta(
        "versioning",
        lTagOf[Option[REFERENCE_VERSION_RULES]],
        false,
        lTagOf[REFERENCE_VERSION_RULES])
    val aggregation: FHIRComponentFieldMeta[LitSeq[RESOURCE_AGGREGATION_MODE]] =
      FHIRComponentFieldMeta(
        "aggregation",
        lTagOf[LitSeq[RESOURCE_AGGREGATION_MODE]],
        false,
        lTagOf[RESOURCE_AGGREGATION_MODE])
  ...
  object Mapping extends CompanionFor[Mapping] {
    ...
    def unapply(o: Mapping): Option[(Option[String], String, Option[String], Id, Option[Code], LitSeq[Extension])] =
      Some((o.id, o.map, o.comment, o.identity, o.language, o.extension))
    val id: FHIRComponentFieldMeta[Option[String]] =
      FHIRComponentFieldMeta("id", lTagOf[Option[String]], false, lTagOf[String])
  ...
  val representation: FHIRComponentFieldMeta[LitSeq[PROPERTY_REPRESENTATION]] =
    FHIRComponentFieldMeta(
      "representation",
      lTagOf[LitSeq[PROPERTY_REPRESENTATION]],
      false,
      lTagOf[PROPERTY_REPRESENTATION])
  ...
  val defaultValue: FHIRComponentFieldMeta[Option[ElementDefinition.DefaultValueChoice]] =
    FHIRComponentFieldMeta(
      "defaultValue",
      lTagOf[Option[ElementDefinition.DefaultValueChoice]],
      true,
  ...
  val slicing: FHIRComponentFieldMeta[Option[ElementDefinition.Slicing]] =
    FHIRComponentFieldMeta(
      "slicing",
      lTagOf[Option[ElementDefinition.Slicing]],
      false,
      lTagOf[ElementDefinition.Slicing])
...
/** Base StructureDefinition for ElementDefinition Type: Captures constraints on each element within the resource,
  * profile, or extension.
  *
  * Subclass of [[core.model.BackboneElement]] (Base StructureDefinition for BackboneElement Type: Base definition for
  * all elements that are defined inside a resource - but not those in a data type.)
  *
  * @constructor
  *   Introduces the fields min, max, path, code, label, short, alias, comment, fixed, sliceName, maxLength, condition,
  *   isSummary, definition, pattern, isModifier, minValue, maxValue, mustSupport, requirements, orderMeaning,
  *   representation, base, defaultValue, contentReference, isModifierReason, example, mapping, `type`,
  *   meaningWhenMissing, sliceIsConstraining, binding, constraint, slicing.
  * @param id
  ...
  * @param path
  *   - The path identifies the element and is expressed as a "."-separated list of ancestor elements, beginning with
  *   the name of the resource or extension.
  * @param code
  *   - A code that has the same meaning as the element in a particular terminology.
  * @param label
  *   - A single preferred label which is the text to display beside the element indicating its meaning or to use to
  ...
  *   properly, exceptions to proper use, etc. (Note: The text you are reading is specified in
  ...
  * @param sliceName
  *   - The name of this element definition slice, when slicing is working. The name must be a token with no dots or
  *   spaces. This is a unique name referring to a specific set of constraints applied to this element, used to provide
  *   a name to different slices of the same element.
  * @param maxLength
  ...
  * @param definition
  *   - Provides a complete explanation of the meaning of the data element for human readability. For the case of
  *   elements derived from existing elements (e.g. constraints), the definition SHALL be consistent with the base
  *   definition, but convey the meaning of the element in the particular context of use of the resource. (Note: The
  *   text you are reading is specified in ElementDefinition.definition).
  * @param pattern
  *   - Specifies a value that the value in the instance SHALL follow - that is, any value in the pattern must be found
  *   in the instance. Other additional values may be found too. This is effectively constraint by example. When
  *   pattern[x] is used to constrain a primitive, it means that the value provided in the pattern[x] must match the
  *   instance value exactly. When pattern[x] is used to constrain an array, it means that each element provided in the
  ...
  *   The effect of this is that the element cannot be ignored by systems: they SHALL either recognize the element and
  *   process it, and/or a pre-determination has been made that it is not relevant to their particular system.
  * @param minValue
  ...
  *   instant, time, decimal, integer, and Quantity.
  * @param mustSupport
  *   - If true, implementations that produce or consume resources SHALL provide "support" for the element in some
  *   meaningful way. If false, the element may be ignored and not supported. If false, whether to populate or use the
  *   data element in any way is at the discretion of the implementation.
  * @param requirements
  *   - This element is for traceability of why the element was created and why the constraints exist as they do. This
  *   may be used to point to source materials or specifications that drove the structure of this element.
  * @param orderMeaning
  ...
  *   deviation of the element through the derived and related profiles. When the element definition is not the original
  *   definition of an element - i.g. either in a constraint on another type, or for elements from a super type in a
  *   snap shot - then the information in provided in the element definition may be different to the base definition. On
  ...
  *   any implementer can define an extension, there is a set of requirements that SHALL be met as part of the
  ...
  *   - The Implicit meaning that is to be understood when this element is missing (e.g. 'when this element is missing,
  *   the period is ongoing').
  * @param sliceIsConstraining
  *   - If true, indicates that this slice definition is constraining a slice definition with the same name in an
  *   inherited profile. If false, the slice is not overriding any slice in an inherited profile. If missing, the slice
  *   might or might not be overriding a slice in an inherited profile, depending on the sliceName.
  * @param binding
  ...
  *   there are multiple different constraints on a single element in the base resource). Slicing can be used in any
  *   resource that has cardinality ..* on the base resource, or any resource with a choice of types. The set of slices
  *   is any elements that come after this in the element sequence that have the same path, until a shorter path occurs
