...
/** A photo, video, or audio recording acquired or used in healthcare. The actual content may be inline or provided by
  ...
  *
  * @constructor
  *   Introduces the fields `type`, view, note, width, partOf, status, issued, device, height, frames, basedOn, subject,
  *   content, modality, operator, bodySite, duration, encounter, identifier, created, reasonCode, deviceName.
  * @param id
  ...
  * @param issued
  *   - The date and time this version of the media was made available to providers, typically after having been
  *   reviewed.
  * @param device
  ...
  * @param frames
  *   - The number of frames in a photo. This is used with a multi-page fax, or an imaging acquisition context that
  *   takes multiple slices in a single image, or an animated gif. If there is more than one frame, this SHALL have a
  *   value in order to alert interface software that a multi-frame capable rendering widget is required.
  * @param basedOn
  ...
  * @param modality
  *   - Details of the type of the media - usually, how it was acquired (what type of device). If images sourced from a
  *   DICOM system, are wrapped in a Media resource, then this is the modality.
  * @param operator
  ...
  *   context of its collection (e.g. series ids) and context ids such as accession numbers or other workflow
  *   identifiers.
  * @param created
