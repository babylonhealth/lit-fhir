...
object MedicinalProduct extends CompanionFor[MedicinalProduct] {
  override val baseType: CompanionFor[MedicinalProduct] = MedicinalProduct
  ...
  override val searchParams: Map[String, MedicinalProduct => Seq[Any]] = Map(
    ...
    "name"          -> (obj => obj.name.map(_.productName).toSeq)
  )
...
/** Detailed definition of a medicinal product, typically for uses other than direct patient care (e.g. regulatory use).
  ...
  * @param masterFile - A master file for to the medicinal product (e.g. Pharmacovigilance System Master File).
  ...
  * @param marketingStatus - Marketing status of the medicinal product, in contrast to marketing authorizaton.
  ...
  * @param additionalMonitoringIndicator - Whether the Medicinal Product is subject to additional monitoring for regulatory reasons.
  * @param combinedPharmaceuticalDoseForm - The dose form for a single part product, or combined form of a multiple part product.
  * @param specialDesignation - Indicates if the medicinal product has an orphan designation for the treatment of a rare disease.
  * @param name - The product's name, including full name and possibly coded parts.
  * @param manufacturingBusinessOperation - An operation applied to the product, for manufacturing or adminsitrative purpose.
  */
@POJOBoilerplate
