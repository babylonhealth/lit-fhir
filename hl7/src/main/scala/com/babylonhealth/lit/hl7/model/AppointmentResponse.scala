...
object AppointmentResponse extends CompanionFor[AppointmentResponse] {
  ...
  override val parentType: CompanionFor[ParentType] = AppointmentResponse
...
/** A reply to an appointment request for a patient and/or practitioner(s), such as a confirmation or rejection.
  ...
  *   of the extension.
  * @param identifier
  *   \- This records identifiers associated with this appointment response concern that are defined by business processes and/ or
  *   used to refer to it when a direct URL reference to the resource itself is not appropriate.
  * @param appointment
  ...
  *   \- Participation status of the participant. When the status is declined or tentative if the start/end times are different to
  *   the appointment, then these times should be interpreted as a requested time change. When the status is accepted, the times
