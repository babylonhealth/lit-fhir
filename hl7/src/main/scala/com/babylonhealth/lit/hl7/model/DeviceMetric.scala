...
object DeviceMetric extends CompanionFor[DeviceMetric] {
  ...
  val calibration: FHIRComponentFieldMeta[LitSeq[DeviceMetric.Calibration]] =
    FHIRComponentFieldMeta(
      "calibration",
      lTagOf[LitSeq[DeviceMetric.Calibration]],
      false,
      lTagOf[DeviceMetric.Calibration])
...
/** Describes a measurement, calculation or setting capability of a medical device.
  ...
  *
  * @constructor
  *   Introduces the fields `type`, unit, color, source, parent, category, identifier, operationalStatus,
  *   measurementPeriod, calibration.
  * @param id
  ...
  * @param unit
  *   - Describes the unit that an observed value determined for this metric will have. For example: Percent, Seconds,
  *   etc.
  * @param color
  ...
  * @param source
  *   - Describes the link to the Device that this DeviceMetric belongs to and that contains administrative device
  *   information such as manufacturer, serial number, etc.
  * @param parent
  *   - Describes the link to the Device that this DeviceMetric belongs to and that provide information about the
  *   location of this DeviceMetric in the containment structure of the parent Device. An example would be a Device that
  *   represents a Channel. This reference can be used by a client application to distinguish DeviceMetrics that have
  ...
  * @param identifier
  *   - Unique instance identifiers assigned to a device by the device or gateway software, manufacturers, other
  ...
  *   - Describes the measurement repetition time. This is not necessarily the same as the update period. The
  *   measurement repetition time can range from milliseconds up to hours. An example for a measurement repetition time
  *   in the range of milliseconds is the sampling rate of an ECG. An example for a measurement repetition time in the
  *   range of hours is a NIBP that is triggered automatically every hour. The update period may be different than the
  *   measurement repetition time, if the device does not update the published observed value with the same frequency as
  *   it was measured.
  * @param calibration
