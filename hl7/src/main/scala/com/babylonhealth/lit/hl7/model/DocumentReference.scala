...
object DocumentReference extends CompanionFor[DocumentReference] {
  ...
  object RelatesTo extends CompanionFor[RelatesTo] {
    implicit def summonObjectAndCompanionRelatesTo_307487467(o: RelatesTo): ObjectAndCompanion[RelatesTo, RelatesTo.type] =
  ...
  object Content extends CompanionFor[Content] {
    ...
    def unapply(o: Content): Option[(Option[String], Option[Coding], LitSeq[Extension], Attachment, LitSeq[Extension])] = Some(
...
/** A reference to a document of any kind for any purpose. Provides metadata about the document so that the document can be
  * discovered and managed. The scope of a document is any seralized object with a mime-type, so includes formal patient centric
  * documents (CDA), cliical notes, scanned paper, and non-patient specific documents like policy text.
  *
  * Subclass of [[hl7.model.DomainResource]] (A resource that includes narrative, extensions, and contained resources.)
  *
  * @constructor
  *   Introduces the fields `type`, date, status, author, subject, category, docStatus, custodian, identifier, description,
  *   authenticator, securityLabel, masterIdentifier, content, context, relatesTo.
  * @param id
