...
object MessageDefinition extends CompanionFor[MessageDefinition] {
  ...
  object AllowedResponse extends CompanionFor[AllowedResponse] {
    ...
    def unapply(o: AllowedResponse): Option[(Option[String], Canonical, LitSeq[Extension], Option[Markdown], LitSeq[Extension])] =
...
/** Defines the characteristics of a message that can be shared between systems, including the type of event that initiates the
  ...
  * @param url
  *   - The business identifier that is used to reference the MessageDefinition and *is* expected to be consistent from server to
  *   server.
  * @param meta
  ...
  * @param name
  *   - A natural language name identifying the message definition. This name should be usable as an identifier for the module by
  *   machine processing applications such as code generation.
  * @param date
  *   - The date (and optionally time) when the message definition was published. The date must change when the business version
  ...
  *   [GraphDefinition](graphdefinition.html) that it controls what resources are to be added to the bundle when building the
  *   document. The GraphDefinition can also specify profiles that apply to the various resources.
  * @param status
  *   - The status of this message definition. Enables tracking the life-cycle of the content.
  * @param parent
  *   - Identifies a protocol or workflow that this MessageDefinition represents a step in.
  * @param version
  *   - The identifier that is used to identify this version of the message definition when it is referenced in a specification,
  *   model, design or instance. This is an arbitrary value managed by the message definition author and is not expected to be
  ...
  *   used to assist with indexing and searching for appropriate message definition instances.
  * @param description
