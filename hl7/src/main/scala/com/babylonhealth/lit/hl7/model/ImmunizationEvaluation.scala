...
/** Describes a comparison of an immunization event against published recommendations to determine if the administration
  ...
  *
  * @constructor
  *   Introduces the fields date, status, series, patient, authority, identifier, doseStatus, description,
  *   targetDisease, doseNumber, seriesDoses, doseStatusReason, immunizationEvent.
  * @param id
