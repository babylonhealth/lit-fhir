...
/** A task to be performed.
  ...
  *
  * @constructor
  *   Introduces the fields `for`, code, note, focus, owner, partOf, status, intent, basedOn, priority, location,
  *   encounter, requester, insurance, identifier, authoredOn, reasonCode, description, statusReason, lastModified,
  *   performerType, businessStatus, instantiatesUri, groupIdentifier, executionPeriod, reasonReference,
  *   relevantHistory, instantiatesCanonical, input, output, restriction.
  * @param id
  ...
  * @param intent
  *   - Indicates the "level" of actionability associated with the Task, i.e. i+R[9]Cs this a proposed task, a planned
  *   task, an actionable task, etc.
  * @param basedOn
  *   - BasedOn refers to a higher-level authorization that triggered the creation of the task. It references a
  *   "request" resource such as a ServiceRequest, MedicationRequest, ServiceRequest, CarePlan, etc. which is distinct
  *   from the "request" resource the task is seeking to fulfill. This latter resource is referenced by FocusOn. For
  *   example, based on a ServiceRequest (= BasedOn), a task is created to fulfill a procedureRequest ( = FocusOn ) to
  ...
  *   to in whole or in part by this Task.
  * @param groupIdentifier
  *   - An identifier that links together multiple tasks and other requests that were created in the same context.
  * @param executionPeriod
  *   - Identifies the time action was first taken against the task (start) and/or the time final action was taken
  *   against the task prior to marking it as completed (end).
  * @param reasonReference
  *   - A resource reference indicating why this task needs to be performed.
  * @param relevantHistory
  *   - Links to Provenance records for past versions of this Task that identify key state transitions or updates that
  ...
  *   - The URL pointing to a *FHIR*-defined protocol, guideline, orderset or other definition that is adhered to in
  ...
  *   be actioned), this element identifies any limitations on what parts of the referenced request should be actioned.
  */
@POJOBoilerplate
class Task(
    override val id: Option[String] = None,
