...
object Subscription extends CompanionFor[Subscription] {
  ...
  object Channel extends CompanionFor[Channel] {
    ...
    val fieldsMeta: Seq[FHIRComponentFieldMeta[_]] =
      Seq(id, `type`, header, payload, endpoint, extension, modifierExtension)
...
/** The subscription resource is used to define a push-based subscription from a server to another system. Once a subscription is registered with the server, the server checks every resource that is created or updated, and if the resource matches the given criteria, it sends a message on the defined "channel" so that another system can take an appropriate action.
  ...
  *
  * @constructor Introduces the fields end, error, status, reason, contact, criteria, channel.
  ...
  * @param error - A record of the last error that occurred when the server processed a notification.
  * @param status - The status of the subscription, which marks the server state for managing the subscription.
  ...
  * @param criteria - The rules that the server should use to determine when to generate notifications for this subscription.
  ...
  * @param channel - Details where to send notifications when resources are received that meet the criteria.
  */
@POJOBoilerplate
class Subscription(
    override val id: Option[String] = None,
