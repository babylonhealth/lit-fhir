...
object SearchParameter extends CompanionFor[SearchParameter] {
  ...
  object Component extends CompanionFor[Component] {
    implicit def summonObjectAndCompanionComponent_2033059278(
...
/** A search parameter that defines a named search item that can be used to search/filter on a resource.
  ...
  *
  * @constructor
  *   Introduces the fields url, name, date, code, base, `type`, xpath, chain, status, target, version, contact,
  *   purpose, modifier, publisher, useContext, expression, xpathUsage, multipleOr, comparator, derivedFrom,
  *   description, multipleAnd, experimental, jurisdiction, component.
  * @param id
  ...
  *   literal address at which at which an authoritative instance of this search parameter is (or will be) published.
  *   This URL can be the target of a canonical reference. It SHALL remain the same when the search parameter is stored
  ...
  * @param name
  *   - A natural language name identifying the search parameter. This name should be usable as an identifier for the
  *   module by machine processing applications such as code generation.
  * @param date
  *   - The date (and optionally time) when the search parameter was published. The date must change when the business
  ...
  * @param version
  *   - The identifier that is used to identify this version of the search parameter when it is referenced in a
  *   specification, model, design or instance. This is an arbitrary value managed by the search parameter author and is
  ...
  *   and may be used to assist with indexing and searching for appropriate search parameter instances.
  * @param expression
  ...
  *   - And how it used.
  * @param multipleAnd
  *   - Whether multiple parameters are allowed - e.g. more than one parameter with the same name. The search matches if
  *   all the parameters match.
  * @param experimental
