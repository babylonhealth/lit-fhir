...
/** An occurrence of information being transmitted; e.g. an alert that was sent to a responsible provider, a public
  ...
  *
  * @constructor
  *   Introduces the fields sent, note, topic, about, partOf, status, medium, sender, basedOn, subject, category,
  *   priority, received, encounter, recipient, identifier, reasonCode, inResponseTo, statusReason, instantiatesUri,
  *   reasonReference, instantiatesCanonical, payload.
  * @param id
  ...
  *   associated.
  * @param recipient
  *   - The entity (e.g. person, organization, clinical information system, care team or device) which was the target of
  *   the communication. If receipts need to be tracked by an individual, a separate resource instance will need to be
  *   created for each recipient. Multiple recipient communications are intended where either receipts are not tracked
  *   (e.g. a mass mail-out) or a receipt is captured in aggregate (all emails confirmed received by a particular time).
  * @param identifier
  *   - Business identifiers assigned to this communication by the performer or other systems which remain constant as
