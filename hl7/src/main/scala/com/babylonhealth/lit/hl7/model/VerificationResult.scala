...
object VerificationResult extends CompanionFor[VerificationResult] {
  ...
  val validator: FHIRComponentFieldMeta[LitSeq[VerificationResult.Validator]] =
    FHIRComponentFieldMeta(
      "validator",
      lTagOf[LitSeq[VerificationResult.Validator]],
      false,
      lTagOf[VerificationResult.Validator])
...
/** Describes validation requirements, source(s), status and dates for one or more elements.
  ...
  *
  * @constructor Introduces the fields need, target, status, frequency, statusDate, lastPerformed, nextScheduled, failureAction, targetLocation, validationType, validationProcess, validator, primarySource, attestation.
  ...
  * @param status - The validation status of the target (attested; validated; in process; requires revalidation; validation failed; revalidation failed).
  ...
  * @param validationType - What the target is validated against (nothing; primary source; multiple sources).
  ...
  * @param validationProcess - The primary process by which the target is validated (edit check; value set; primary source; multiple sources; standalone; in context).
