...
object Appointment extends CompanionFor[Appointment] {
  implicit def summonObjectAndCompanionAppointment1671086218(o: Appointment): ObjectAndCompanion[Appointment, Appointment.type] =
  ...
  object Participant extends CompanionFor[Participant] {
    implicit def summonObjectAndCompanionParticipant385746686(o: Participant): ObjectAndCompanion[Participant, Participant.type] =
      ObjectAndCompanion(o, this)
...
/** A booking of a healthcare event among patient(s), practitioner(s), related person(s) and/or device(s) for a specific
  ...
  *   For example, where the actual time of appointment is only an estimate or if a 30 minute appointment is being requested, but
  *   any time would work. Also, if there is, for example, a planned 15 minute break in the middle of a long appointment, the
