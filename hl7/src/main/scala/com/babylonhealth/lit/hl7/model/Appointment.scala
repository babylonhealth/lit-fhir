...
/** A booking of a healthcare event among patient(s), practitioner(s), related person(s) and/or device(s) for a specific
  ...
  *
  * @constructor
  *   Introduces the fields end, slot, start, status, created, comment, basedOn, priority, specialty, identifier,
  *   reasonCode, serviceType, description, serviceCategory, appointmentType, reasonReference, minutesDuration,
  *   requestedPeriod, cancelationReason, patientInstruction, supportingInformation, participant.
  * @param id
  ...
  * @param created
  *   - The date that this appointment was initially created. This could be different to the meta.lastModified value on
  *   the initial entry, as this could have been before the resource was created on the FHIR server, and should remain
  ...
  * @param identifier
  *   - This records identifiers associated with this appointment concern that are defined by business processes and/or
  *   used to refer to it when a direct URL reference to the resource itself is not appropriate (e.g. in CDA documents,
  ...
  * @param description
  *   - The brief description of the appointment as would be shown on a subject line in a meeting request, or
  *   appointment list. Detailed or expanded information should be put in the comment field.
  * @param implicitRules
  ...
  *   When the patient arrives and the encounter begins it may be used as the admission diagnosis. The indication will
  *   typically be a Condition (with other resources referenced in the evidence.detail), or a Procedure.
  * @param minutesDuration
  *   - Number of minutes that the appointment is to take. This can be less than the duration between the start and end
  *   times. For example, where the actual time of appointment is only an estimate or if a 30 minute appointment is
  *   being requested, but any time would work. Also, if there is, for example, a planned 15 minute break in the middle
  *   of a long appointment, the duration may be 15 minutes less than the difference between the start and end.
  * @param requestedPeriod
  *   - A set of date ranges (potentially including times) that the appointment is preferred to be scheduled within. The
  *   duration (usually in minutes) could also be provided to indicate the length of the appointment to fill and
  ...
  *   - The coded reason for the appointment being cancelled. This is often used in reporting/billing/futher processing
  *   to determine if further actions are required, or specific fees apply.
  * @param patientInstruction
  *   - While Appointment.comment contains information for internal use, Appointment.patientInstructions is used to
  *   capture patient facing information about the Appointment (e.g. please bring your referral or fast from 8pm night
  *   before).
