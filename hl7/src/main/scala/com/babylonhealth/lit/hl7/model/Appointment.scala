...
object Appointment extends CompanionFor[Appointment] {
  implicit def summonObjectAndCompanionAppointment613298086(
      o: Appointment): ObjectAndCompanion[Appointment, Appointment.type] = ObjectAndCompanion(o, this)
  ...
  val participant: FHIRComponentFieldMeta[NonEmptyLitSeq[Appointment.Participant]] =
    FHIRComponentFieldMeta(
      "participant",
      lTagOf[NonEmptyLitSeq[Appointment.Participant]],
      false,
      lTagOf[Appointment.Participant])
  ...
  override val searchParams: Map[String, Appointment => Seq[Any]] = Map(
    ...
    "practitioner" -> (obj =>
      obj.participant.flatMap(_.actor).filter(_.reference.exists(_.contains("Practitioner/"))).toSeq),
...
/** A booking of a healthcare event among patient(s), practitioner(s), related person(s) and/or device(s) for a specific date/time. This may result in one or more Encounter(s).
  ...
  *
  * @constructor Introduces the fields end, slot, start, status, created, comment, basedOn, priority, specialty, identifier, reasonCode, serviceType, description, serviceCategory, appointmentType, reasonReference, minutesDuration, requestedPeriod, cancelationReason, patientInstruction, supportingInformation, participant.
  ...
  * @param slot - The slots from the participants' schedules that will be filled by the appointment.
  * @param start - Date/Time that the appointment is to take place.
  * @param status - The overall status of the Appointment. Each of the participants has their own participation status which indicates their involvement in the process, however this status indicates the shared status.
  * @param created - The date that this appointment was initially created. This could be different to the meta.lastModified value on the initial entry, as this could have been before the resource was created on the FHIR server, and should remain unchanged over the lifespan of the appointment.
  * @param comment - Additional comments about the appointment.
  * @param basedOn - The service request this appointment is allocated to assess (e.g. incoming referral or procedure request).
  * @param language - The base language in which the resource is written.
  * @param priority - The priority of the appointment. Can be used to make informed decisions if needing to re-prioritize appointments. (The iCal Standard specifies 0 as undefined, 1 as highest, 9 as lowest priority).
  ...
  * @param identifier - This records identifiers associated with this appointment concern that are defined by business processes and/or used to refer to it when a direct URL reference to the resource itself is not appropriate (e.g. in CDA documents, or in written / printed documentation).
  * @param reasonCode - The coded reason that this appointment is being scheduled. This is more clinical than administrative.
  * @param serviceType - The specific service that is to be performed during this appointment.
  * @param description - The brief description of the appointment as would be shown on a subject line in a meeting request, or appointment list. Detailed or expanded information should be put in the comment field.
  ...
  * @param reasonReference - Reason the appointment has been scheduled to take place, as specified using information from another resource. When the patient arrives and the encounter begins it may be used as the admission diagnosis. The indication will typically be a Condition (with other resources referenced in the evidence.detail), or a Procedure.
  * @param minutesDuration - Number of minutes that the appointment is to take. This can be less than the duration between the start and end times.  For example, where the actual time of appointment is only an estimate or if a 30 minute appointment is being requested, but any time would work.  Also, if there is, for example, a planned 15 minute break in the middle of a long appointment, the duration may be 15 minutes less than the difference between the start and end.
  * @param requestedPeriod - A set of date ranges (potentially including times) that the appointment is preferred to be scheduled within.
  ...
  * @param cancelationReason - The coded reason for the appointment being cancelled. This is often used in reporting/billing/futher processing to determine if further actions are required, or specific fees apply.
  * @param patientInstruction - While Appointment.comment contains information for internal use, Appointment.patientInstructions is used to capture patient facing information about the Appointment (e.g. please bring your referral or fast from 8pm night before).
  * @param supportingInformation - Additional information to support the appointment provided when making the appointment.
