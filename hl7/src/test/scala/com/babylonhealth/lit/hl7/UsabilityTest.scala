...
class UsabilityTest extends AnyFreeSpec with Matchers {
  "can stringify FHIR without json serdes" in {
    ...
    """Observation(id = Some(9f6ba48aa7ed06534520c16546df36fa), code = CodeableConcept(text = Some(Serum bilirubin level), coding = LitSeq(Coding(code = Some(44E..), system = Some(CTV3), display = Some(Serum bilirubin level)), Coding(code = Some(999691000000104), system = Some(SNOMED)))), status = UNKNOWN, value[x]: Quantity = Choice[_$U](Quantity, Quantity(unit = Some(umol/L), value = Some(5.0))), effective[x]: DateTime = Choice[_$U](DateTime, FHIRDateTime(2019-11-07T17:27Z,Time)))"""
