...
import com.babylonhealth.lit.uscore.model.Us_core_condition
...
class ErrorMessageTest extends AnyFreeSpec with Matchers with Inside {
  "error message" - {
    ...
    "should mention interesting target types like `Us_core_condition`" in {
      val msg = errorMessageWhenDecoding[Us_core_condition]("""{"resourceType": "Condition"}""")
      msg should include("Condition as Us_core_condition")
    }
