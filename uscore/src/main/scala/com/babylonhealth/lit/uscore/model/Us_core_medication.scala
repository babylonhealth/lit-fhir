...
object Us_core_medication extends CompanionFor[Us_core_medication] {
  implicit def summonObjectAndCompanionUs_core_medication133309565(
      o: Us_core_medication): ObjectAndCompanion[Us_core_medication, Us_core_medication.type] =
...
/** Defines constraints and extensions on the Medication resource for the minimal set of data to query and retrieve
  * patient retrieving patient's medication information.
  *
  * Subclass of [[hl7.model.Medication]] (This resource is primarily used for the identification and definition of a
  * medication for the purposes of prescribing, dispensing, and administering a medication as well as for making
  ...
  * @param code
  *   - A code (or set of codes) that specify this medication, or a textual description if no code is available. Usage
  *   note: This could be a standard medication code such as a code from RxNorm, SNOMED CT, IDMP etc. It could also be a
  *   national or local formulary code, optionally with translations to other code systems.
  * @param form
