...
/** Defines constraints and extensions on the Device resource for the minimal set of data to query and retrieve a
  * patient's implantable device(s).
  *
  * Subclass of [[hl7.model.Device]] (A type of a manufactured item that is used in the provision of healthcare without
  * being substantially changed through that activity. The device may be a medical or non-medical device.)
  *
  * @constructor
