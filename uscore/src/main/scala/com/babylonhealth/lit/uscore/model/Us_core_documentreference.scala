...
/** The document reference profile used in US Core.
  *
  * Subclass of [[hl7.model.DocumentReference]] (A reference to a document of any kind for any purpose. Provides
  * metadata about the document so that the document can be discovered and managed. The scope of a document is any
  * seralized object with a mime-type, so includes formal patient centric documents (CDA), cliical notes, scanned paper,
  ...
  * @param `type`
  *   - Specifies the particular kind of document referenced (e.g. History and Physical, Discharge Summary, Progress
  *   Note). This usually equates to the purpose of making the document referenced.
  * @param date
  ...
  * @param subject
  *   - Who or what the document is about. The document can be about a person, (patient or healthcare practitioner), a
  *   device (e.g. a machine) or even a group of subjects (such as a document about a herd of farm animals, or a set of
  *   patients that share a common exposure).
  * @param language
  *   - The base language in which the resource is written.
  * @param category
  *   - A categorization for the type of document referenced - helps for indexing and searching. This may be implied by
  ...
  *   DocumentReference.securityLabel contains a snapshot of the security labels on the document the reference refers
  *   to.
  * @param masterIdentifier
  *   - Document identifier as assigned by the source of the document. This identifier is specific to this version of
  *   the document. This unique identifier may be used elsewhere to identify this version of the document.
