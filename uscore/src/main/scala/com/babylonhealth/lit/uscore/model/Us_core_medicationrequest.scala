...
/** Defines constraints and extensions on the MedicationRequest resource for the minimal set of data to query and
  * retrieve prescription information.
  *
  * Subclass of [[hl7.model.MedicationRequest]] (An order or request for both supply of the medication and the
  * instructions for administration of the medication to a patient. The resource is called "MedicationRequest" rather
  * than "MedicationPrescription" or "MedicationOrder" to generalize the use across inpatient and outpatient settings,
  * including care plans, etc., and to harmonize with workflow patterns.)
  *
  * @constructor
  *   Inherits all params from parent. Requires the following fields which were optional in the parent: requester,
  *   authoredOn.
  * @param id
  ...
  * @param category
  *   - Indicates the type of medication request (for example, where the medication is expected to be consumed or
  *   administered (i.e. inpatient or outpatient)).
  * @param priority
  *   - Indicates how quickly the Medication Request should be addressed with respect to other requests.
  * @param recorder
  *   - The person who entered the order on behalf of another individual for example in the case of a verbal or a
  *   telephone order.
  * @param contained
  ...
  *   administration).
  * @param insurance
  *   - Insurance plans, coverage extensions, pre-authorizations and/or pre-determinations that may be required for
  *   delivering the requested service.
  * @param identifier
  *   - Identifiers associated with this medication request that are defined by business processes and/or used to refer
  *   to it when a direct URL reference to the resource itself is not appropriate. They are business identifiers
  *   assigned to this resource by the performer or other systems and remain constant as the resource is updated and
  ...
  * @param reported
  *   - Indicates if this record was captured as a secondary 'reported' record rather than as an original primary
  ...
  * @param eventHistory
  *   - Links to Provenance records for past versions of this resource or fulfilling request or event resources that
  ...
  * @param medication
  *   - Identifies the medication being requested. This is a link to a resource that represents the medication which may
  *   be the details of the medication or simply an attribute carrying a code that identifies the medication from a
  ...
  *   for a patient; e.g. Drug-drug interaction, duplicate therapy, dosage alert etc.
  ...
  *   - Include additional information (for example, patient height and weight) that supports the ordering of the
  *   medication.
  ...
  *   There may be in some settings (e.g. hospitals) institutional or system support for completing the dispense details
