...
object Us_core_observation_lab extends CompanionFor[Us_core_observation_lab] {
  ...
  val effective: FHIRComponentFieldMeta[Option[Us_core_observation_lab.EffectiveChoice]] =
    FHIRComponentFieldMeta("effective", lTagOf[Option[Us_core_observation_lab.EffectiveChoice]], true, lTagOf[Union_0934386166])
...
/** Defines constraints and extensions on the Observation resource for the minimal set of data to query and retrieve laboratory
  * test results
  *
  ...
  *   CT](http://hl7.org/fhir/ValueSet/uslab-obs-codedresults). If a numeric value, valueQuantity.code **SHALL** be selected from
  *   [UCUM](http://unitsofmeasure.org). A FHIR [UCUM Codes value set](http://hl7.org/fhir/STU3/valueset-ucum-units.html) that
  *   defines all UCUM codes is in the FHIR specification.
  * @param bodySite
  ...
  * @param effective
  *   - For lab tests this is the specimen collection date. For Ask at Order Entry Questions (AOE)'s this is the date the question
  *   was asked.
  * @param implicitRules
