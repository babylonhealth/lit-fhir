...
object Us_core_observation_lab extends CompanionFor[Us_core_observation_lab] {
  ...
  override def fields(t: Us_core_observation_lab): Seq[FHIRComponentField[_]] = Seq(
...
/** Defines constraints and extensions on the Observation resource for the minimal set of data to query and retrieve laboratory test results
  ...
  *              Requires the following fields which were optional in the parent: subject, category.
  ...
  * @param code - The test that was performed.  A LOINC **SHALL** be used if the concept is present in LOINC.
  ...
  * @param effective - For lab tests this is the specimen collection date.  For Ask at Order Entry Questions (AOE)'s this is the date the question was asked.
