...
object Us_core_observation_lab extends CompanionFor[Us_core_observation_lab] {
  ...
  def apply(
      ...
      meta: Option[Meta] = Some(
        new Meta(profile = LitSeq("http://hl7.org/fhir/us/core/StructureDefinition/us-core-observation-lab"))),
      ...
      effective: Option[Us_core_observation_lab.EffectiveChoice] = None,
  ...
  ): Us_core_observation_lab = new Us_core_observation_lab(
    id,
    meta,
    text,
    code,
  ...
  val value: FHIRComponentFieldMeta[Option[Us_core_observation_lab.ValueChoice]] =
    FHIRComponentFieldMeta("value", lTagOf[Option[Us_core_observation_lab.ValueChoice]], true, lTagOf[Union_0802685816])
  ...
  val effective: FHIRComponentFieldMeta[Option[Us_core_observation_lab.EffectiveChoice]] =
...
/** Defines constraints and extensions on the Observation resource for the minimal set of data to query and retrieve
  ...
  *   optional in the parent: subject, category.
  * @param id
  ...
  * @param value
  *   - The Laboratory result value. If a coded value, the valueCodeableConcept.code **SHOULD** be selected from [SNOMED
  *   CT](http://hl7.org/fhir/ValueSet/uslab-obs-codedresults). If a numeric value, valueQuantity.code **SHALL** be
  *   selected from [UCUM](http://unitsofmeasure.org). A FHIR [UCUM Codes value
  *   set](http://hl7.org/fhir/STU3/valueset-ucum-units.html) that defines all UCUM codes is in the FHIR specification.
  * @param bodySite
  ...
  * @param effective
  *   - For lab tests this is the specimen collection date. For Ask at Order Entry Questions (AOE)'s this is the date
