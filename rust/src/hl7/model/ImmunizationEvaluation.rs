...
impl ImmunizationEvaluation for ImmunizationEvaluationRaw {
  ...
  fn series(&self) -> Option<&String> { self.series.as_ref() }
  ...
  fn doseNumber(&self) -> Option<&UnionPositiveIntOrString> { self.doseNumber.as_ref() }
  fn seriesDoses(&self) -> Option<&UnionPositiveIntOrString> { self.seriesDoses.as_ref() }
