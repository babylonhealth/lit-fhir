...
impl RiskEvidenceSynthesis for RiskEvidenceSynthesisRaw {
  ...
  fn exposure(&self) -> Option<&Box<dyn Reference>> { self.exposure.as_ref() }
  ...
  fn sampleSize(&self) -> Option<&Box<RiskEvidenceSynthesis_SampleSize>> { self.sampleSize.as_ref() }
  fn riskEstimate(&self) -> Option<&Box<RiskEvidenceSynthesis_RiskEstimate>> { self.riskEstimate.as_ref() }
