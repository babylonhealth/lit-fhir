...
/** Base StructureDefinition for Reference Type: A reference from one resource to another.
  ...
  *     reference may be version specific or not. If the reference is not to a FHIR RESTful server, then it should be assumed to
  *     be version specific. Internal fragment references (start with '#') refer to contained resources.
  * @param identifier
  ...
  *     something that is actually exposed as a FHIR instance, but it SHALL point to a business concept that would be expected to
  *     be exposed as a FHIR instance, and that instance would need to be of a FHIR resource type allowed by the reference.
  */
@POJOBoilerplate
class Reference(
