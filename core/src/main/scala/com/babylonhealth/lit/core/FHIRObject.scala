...
object FHIRObject {
  ...
  def attrMap(el: (FHIRComponentFieldMeta[_], PrimitiveElementInfo)*): TreeMap[FHIRComponentFieldMeta[_], PrimitiveElementInfo] =
...
abstract class FHIRObjectRaw[Stage <: LifecycleStage](
  ...
  val companion: CompanionFor[this.type] =
    Class.forName(this.getClass.getName + "$").getField("MODULE$").get(null).asInstanceOf[CompanionFor[this.type]]
  // For HL7 classes, this should be the same as `thisTypeName`, but will differ for domain-specific subclasses
  def thisClassName: String = this.getClass.getSimpleName
  override def equals(obj: Any): Boolean = obj match {
    case x: FHIRObject if this eq x => true
    case x: FHIRObject if x.thisTypeName == this.thisTypeName =>
  ...
  override def hashCode(): Int =
    companion.baseType.fields(this).foldLeft(primitiveAttributes.##)((acc, next) => 31 * acc + next.value.##)
  // End maybe-should-all-be-autogenerated
  ...
  protected final def suffCond[T](implicit tt: LTag[T]): (Option[String], (Any, LTag[_]) => Boolean) = {
    ...
    val cond: (Any, LTag[_]) => Boolean =
      ...
      if (suff exists stringSubSuffixes) (cand, t) => cand.isInstanceOf[String] && t.tag =:= tt.tag
        ...
        val clazz: Class[_] = Class forName tt.tag.repr
  ...
  /** Bit faster than `>>`, but still much slower than using `update$foo` when possible. If you _must_ use it, then:
    *   - `T` should not be a Choice[_], a LitSeq[_], an Option[_], or any 'subtyped' type (eg PositiveInt). You should ensure, if
    *     T is a supertype of multiple valid choice values (e.g. T =:= Object), that the return value of fn retains the same type
    *     as the input value.
    */
  ...
  /** Extract values of type From Unlike nodalMap, this is safe even if From is a Choice[_], a LitSeq[_] or an Option[_], however
    * there remains a caveat with 'subtyped' types (eg PositiveInt), in that we can't differentiate them from the parent class
