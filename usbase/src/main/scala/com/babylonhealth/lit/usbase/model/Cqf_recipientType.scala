...
object Cqf_recipientType extends CompanionFor[Cqf_recipientType] {
  implicit def summonObjectAndCompanionCqf_recipientType1012652649(
  ...
  ): Cqf_recipientType = new Cqf_recipientType(
    id,
    value,
...
/** The type of individual that will consume the response content. This may be different from the requesting user type (e.g. if a
  * clinician is getting disease management guidance for provision to a patient). E.g. patient, healthcare provider or specific
