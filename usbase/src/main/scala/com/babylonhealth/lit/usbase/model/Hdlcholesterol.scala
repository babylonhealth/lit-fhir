...
object Hdlcholesterol extends CompanionFor[Hdlcholesterol] {
  implicit def summonObjectAndCompanionHdlcholesterol147159219(
  ...
  val effective: FHIRComponentFieldMeta[Option[Hdlcholesterol.EffectiveChoice]] =
    FHIRComponentFieldMeta("effective", lTagOf[Option[Hdlcholesterol.EffectiveChoice]], true, lTagOf[Union_1926447453])
  ...
  def extractValue(t: Hdlcholesterol): Option[Quantity]                           = t.value.map(x => x.toSubRefNonUnion[Quantity])
...
class Hdlcholesterol(
    ...
    override val meta: Option[Meta] = Some(new Meta(profile = LitSeq("http://hl7.org/fhir/StructureDefinition/hdlcholesterol"))),
