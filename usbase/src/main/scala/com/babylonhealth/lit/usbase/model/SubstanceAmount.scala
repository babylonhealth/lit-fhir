...
object SubstanceAmount extends CompanionFor[SubstanceAmount] {
  ...
  object ReferenceRange extends CompanionFor[ReferenceRange] {
    ...
    def unapply(o: ReferenceRange): Option[(Option[String], Option[Quantity], LitSeq[Extension], Option[Quantity])] =
      Some((o.id, o.lowLimit, o.extension, o.highLimit))
...
/** Base StructureDefinition for SubstanceAmount Type: Chemical substances are a single substance type whose primary
  * defining element is the molecular structure. Chemical substances shall be defined on the basis of their complete
  * covalent molecular structure; the presence of a salt (counter-ion) and/or solvates (water, alcohols) is also
  * captured. Purity, grade, physical form or particle size are not taken into account in the definition of a chemical
  ...
  *   actual relative amounts of substances or molecular fragments it is essential to indicate whether the amount refers
  *   to a mole ratio or weight ratio. For any given element an effort should be made to use same the amount type for
  *   all related definitional elements.
  * @param amountText
