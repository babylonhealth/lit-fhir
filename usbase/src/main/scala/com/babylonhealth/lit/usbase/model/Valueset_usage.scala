...
object Valueset_usage extends CompanionFor[Valueset_usage] {
  implicit def summonObjectAndCompanionValueset_usage1305398519(
      o: Valueset_usage): ObjectAndCompanion[Valueset_usage, Valueset_usage.type] = ObjectAndCompanion(o, this)
  override type ResourceType = Extension
  override type ParentType   = Extension
  override val baseType: CompanionFor[ResourceType] = Extension
  override val parentType: CompanionFor[ParentType] = Extension
  override val profileUrl: Option[String]           = Some("http://hl7.org/fhir/StructureDefinition/valueset-usage")
  def apply(
      id: Option[String] = None,
      extension: NonEmptyLitSeq[Extension],
      primitiveAttributes: TreeMap[FHIRComponentFieldMeta[_], PrimitiveElementInfo] = FHIRObject.emptyAtts
  ): Valueset_usage = new Valueset_usage(
    id,
    extension,
  ...
  override def fields(t: Valueset_usage): Seq[FHIRComponentField[_]] = fieldsFromParent(t).get
  def extractId(t: Valueset_usage): Option[String]                   = t.id
  def extractExtension(t: Valueset_usage): NonEmptyLitSeq[Extension] = t.extension.asNonEmpty
  override val thisName: String                                      = "Valueset_usage"
  override val searchParams: Map[String, Valueset_usage => Seq[Any]] = Extension.searchParams
  def unapply(o: Valueset_usage): Option[(Option[String], NonEmptyLitSeq[Extension])] = Some(
    (o.id, o.extension.asNonEmpty))
  def decodeThis(cursor: HCursor)(implicit params: DecoderParams): Try[Valueset_usage] =
...
/** Consumers of the value set and the implementations, projects or standards that the author has utilized the value set
  * in.
  *
