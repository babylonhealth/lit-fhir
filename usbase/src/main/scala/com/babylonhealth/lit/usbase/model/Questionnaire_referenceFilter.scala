...
object Questionnaire_referenceFilter extends CompanionFor[Questionnaire_referenceFilter] {
  ...
  ): Questionnaire_referenceFilter = new Questionnaire_referenceFilter(
    id,
    url,
    value,
  ...
  def extractUrl(t: Questionnaire_referenceFilter): String                          = t.url
  ...
  def unapply(o: Questionnaire_referenceFilter): Option[(Option[String], String, String)] = Some(
  ...
  def decodeThis(cursor: HCursor)(implicit params: DecoderParams): Try[Questionnaire_referenceFilter] =
    checkUnknownFields(cursor, otherMetas, refMetas) flatMap (_ =>
      Try(
        new Questionnaire_referenceFilter(
...
/** Identifies a filter to apply when looking up candidate answers for the question.
  ...
  */
@POJOBoilerplate
class Questionnaire_referenceFilter(
