...
object Workflow_supportingInfo extends CompanionFor[Workflow_supportingInfo] {
  implicit def summonObjectAndCompanionWorkflow_supportingInfo853581146(
  ...
  override def fields(t: Workflow_supportingInfo): Seq[FHIRComponentField[_]] = fieldsFromParent(t).get
  ...
  def unapply(o: Workflow_supportingInfo): Option[(Option[String], Reference)] = Some(
...
/** Other resources *from the patient record* that may be relevant to the event. The information from these resources was either
  * used to create the instance or is provided to help with its interpretation. This extension **should not** be used if more
  * specific inline elements or extensions are available. For example, use `Observation.hasMember` instead of
  * supportingInformation for representing the members of an Observation panel.
  *
