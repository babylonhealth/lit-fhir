...
object Encounter_reasonCancelled extends CompanionFor[Encounter_reasonCancelled] {
  implicit def summonObjectAndCompanionEncounter_reasonCancelled_2057882466(
  ...
  ): Encounter_reasonCancelled = new Encounter_reasonCancelled(
    id,
    value,
  ...
  override def fields(t: Encounter_reasonCancelled): Seq[FHIRComponentField[_]] = fieldsFromParent(t).get
  def extractId(t: Encounter_reasonCancelled): Option[String]                   = t.id
  def extractValue(t: Encounter_reasonCancelled): CodeableConcept               = t.value.get.toSubRefNonUnion[CodeableConcept]
  override val thisName: String                                                 = "Encounter_reasonCancelled"
  override val searchParams: Map[String, Encounter_reasonCancelled => Seq[Any]] = Extension.searchParams
  def unapply(o: Encounter_reasonCancelled): Option[(Option[String], CodeableConcept)] = Some(
    (o.id, o.value.get.toSubRefNonUnion[CodeableConcept]))
  def decodeThis(cursor: HCursor)(implicit params: DecoderParams): Try[Encounter_reasonCancelled] =
    checkUnknownFields(cursor, otherMetas, refMetas) flatMap (_ =>
      Try(
        new Encounter_reasonCancelled(
...
/** If the encountered was cancelled after it was planned, why? Applies only if the status is cancelled.
  ...
  */
@POJOBoilerplate
class Encounter_reasonCancelled(
