...
object Provenance_relevant_history extends CompanionFor[Provenance_relevant_history] {
  ...
  override def fields(t: Provenance_relevant_history): Seq[FHIRComponentField[_]] = Seq(
...
/** Guidance on using Provenance for related history elements
  ...
  *              Refines the types of: occurred.
  *              Requires the following fields which were optional in the parent: activity, occurred.
  ...
  * @param target - This points to the version of the resource that was created as a result of this historical record action.  Typically only a single resource will be referenced, but if the same action is performed to multiple resources in the context of a transaction, some systems may link multiple resource instances.
  ...
  * @param occurred - This indicates the time the resource action (creation, revision, deletion, etc.) occurred.
