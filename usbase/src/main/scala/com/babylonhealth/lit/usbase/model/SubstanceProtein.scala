...
/** A SubstanceProtein is defined as a single unit of a linear amino acid sequence, or a combination of subunits that
  ...
  *   subunit number and position within subunit followed by the abbreviation of the amino acids involved. The disulfide
  *   linkage positions shall actually contain the amino acid Cysteine at the respective positions.
  ...
  *   finished SubstanceProtein is a complex of multiple sequences; subunits are not used to delineate domains within a
  *   single sequence. Subunits are listed in order of decreasing length; sequences of the same length will be ordered
  *   by decreasing molecular weight; subunits that have identical sequences will be repeated multiple times.
  */
@POJOBoilerplate
