...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:NamingSystem/f:identifier/f:period">
    ...
    <sch:rule context="f:NamingSystem/f:identifier/f:assigner">
    ...
    <sch:rule context="f:NamingSystem/f:uniqueId/f:authoritative">
      <sch:assert test="count(f:uniqueID)=count(distinct-values(f:uniqueID/f:period/@value)) or not(exists(for $type in distinct-values(f:uniqueId/f:type/@value) return if (count(f:uniqueId[f:type/@value=$type and f:authoritative/@value=true()])&gt;1) then $type else ()))">nsd-3: Can't have more than one authoritative identifier for a type/period combination (only one authoritative identifier allowed at any given point of time)</sch:assert>
    </sch:rule>
  </sch:pattern>
</sch:schema>
